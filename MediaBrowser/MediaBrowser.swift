...
@objcMembers public class MediaBrowser: UIViewController, AVPlayerViewControllerDelegate {
    ...
    var recycledPages = Set<MediaZoomingScrollView>()
    var currentPageIndex = 0
    var previousPageIndex = Int.max
    var previousLayoutBounds = CGRect.zero
    ...
    var previousNavigationBarTranslucent = true
    var previousNavigationBarStyle = UIBarStyle.default
    var previousNavigationBarTextColor: UIColor?
    var previousNavigationBarBackgroundColor: UIColor?
    var previousNavigationBarTintColor: UIColor?
    var previousViewControllerBackButton: UIBarButtonItem?
    var previousStatusBarStyle: UIStatusBarStyle = .default

    var controlsTap: UIButton?

    // Video
    lazy var currentVideoPlayerViewController: MediaBrowserAVPlayerViewController = {
    ...
    public var scrollViewBackgroundColor = UIColor.systemBackground
    ...
    public var navigationBarTextColor = UIColor.label

    /// UINavigationBar Background Color for MediaBrowser
    public var navigationBarBackgroundColor = UIColor.systemBackground

    /// UINavigationBar Tint Color for MediaBrowser
    public var navigationBarTintColor = UIColor.systemBackground
    ...
    public var toolbarTextColor = UIColor.label

    /// UIToolBar Tint Color for MediaBrowser
    public var toolbarBarTintColor = UIColor.systemBackground

    /// UIToolBar Tint Background for MediaBrowser
    public var toolbarBackgroundColor = UIColor.systemBackground
    ...
    public var loadingIndicatorInnerRingColor = UIColor.label

    /// Loading Indicator Outer Ring Color
    public var loadingIndicatorOuterRingColor = UIColor.systemBackground
    ...
    public var loadingIndicatorFontColor = UIColor.label
    ...
    public func toggleActionButton(direction: Bool) {
        DispatchQueue.main.async {
            if direction {
                self.actionButton = UIBarButtonItem(
                    barButtonSystemItem: UIBarButtonItem.SystemItem.play,
                    target: self,
...
//                }
            ...
            // Right - Action
            items.append(self.actionButton!)
     ...
     */
    override public init(nibName: String?, bundle nibBundle: Bundle?) {
    ...
    open func performLayout() {
        ...
        // Right - Action
        if actionButton != nil && !(!hasItems && navigationItem.rightBarButtonItem == nil) {
    ...
    override open func willMove(toParent parent: UIViewController?) {
        ...
        if let navBar = navigationController?.navigationBar, didSavePreviousStateOfNavBar, parent == nil {
            navBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: previousNavigationBarTextColor ?? UIColor.systemBackground]
        ...
        if index < mediaArray.count && index >= 0 {
            if mediaArray[index] == nil {
                if let d = delegate {
                    ...
                    if media == nil && fixedMediasArray != nil && index < fixedMediasArray!.count {
                        media = fixedMediasArray![index]
                    }
                    media?.placeholderImage = placeholderImage?.image
    ...
    func captionViewForPhotoAtIndex(index: Int) -> MediaCaptionView? {
        ...
        if let d = delegate {
            ...
            if let p = mediaAtIndex(index: index), captionView == nil {
    ...
    var frameForPagingScrollView: CGRect {
        ...
        if areControlsHidden {
            if let navi = navigationController?.navigationBar.frame {
    ...
    open func updateNavigation() {
        ...
        if let gc = gridController {
            if gc.selectionMode {
                title = NSLocalizedString("Select Photos", comment: "")
    ...
    func playVideo(videoURL: URL, atPhotoIndex _: Int) {
        // Setup player
    ...
    func videoFinishedCallback(notification _: NSNotification) {
        if let player = currentVideoPlayerViewController.player {
            if let d = delegate {
    ...
    func setVideoLoadingIndicatorVisible(visible: Bool, atPageIndex _: Int) {
    ...
    func hideGrid() {
        if let gc = gridController {
            ...
            pagingScrollView.frame = pagingFrame.offsetBy(
                dx: 0,
                dy: (startOnGrid ? 1 : -1) * pagingFrame.size.height
            )
    ...
    func setControlsHidden(hidden: Bool, animated: Bool, permanent: Bool) {
        // Force visible
        var hidden = hidden
        if numberOfMedias == 0 || gridController != nil || alwaysShowControls {
            hidden = false
        }
        ...
        if animated {
            ...
            if hidden {
                toolbar.frame = toolbar.frame.offsetBy(dx: 0, dy: animatonOffset)
            }
            toolbar.alpha = hidden ? 0.0 : toolbarAlpha
            ...
            for page in visiblePages {
                if let v = page.captionView {
                    ...
                    v.frame = captionFrame
                    v.alpha = hidden ? 0.0 : captionAlpha
                }
            }
            ...
            for page in visiblePages {
                if let button = page.selectedButton {
                    let v = button
                    var newFrame = frameForSelectedButton(selectedButton: v, atIndex: 0)
                    newFrame.origin.x = v.frame.origin.x
                    v.frame = newFrame
                }
            }
    ...
    func doneButtonPressed(sender _: AnyObject) {
    ...
    func defaultActionForMedia(atIndex index: Int) {
        ...
        if let photo = mediaAtIndex(index: index) {
            if numberOfMedias > 0 && photo.underlyingImage != nil {
                ...
                // Show
                if let vc = activityViewController {
