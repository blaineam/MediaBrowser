...
class MediaZoomingScrollView: UIScrollView, UIScrollViewDelegate, TapDetectingImageViewDelegate, TapDetectingViewDelegate {
    ...
    init(mediaBrowser: MediaBrowser) {
        ...
        loadingIndicator.innerRingColor = UIColor.label
        loadingIndicator.outerRingColor = UIColor.systemBackground
...
//        photo = nil
        ...
        if let placeholder = mediaBrowser.placeholderImage, placeholder.isAppliedForAll || (!placeholder.isAppliedForAll && self.index == self.mediaBrowser.currentIndex) {
            photoImageView.image = mediaBrowser.placeholderImage?.image
        } else {
    ...
    func displayImage() {
        if media != nil && (photoImageView.image == nil || photoImageView.image === mediaBrowser.placeholderImage?.image) {
            // Reset
    ...
    func handleSingleTap(touchPoint _: CGPoint) {
        mediaBrowser.perform(#selector(MediaBrowser.toggleControls), with: nil, afterDelay: 0.5)
    }
    ...
    func tripleTapDetectedInImageView(view _: UIImageView, touch _: UITouch) {}
...
//        touchY += self.contentOffset.y
    ...
    func doubleTapDetectedInView(view: UIView, touch: UITouch) {
        ...
        touchX *= 1.0 / zoomScale
        touchY *= 1.0 / zoomScale
    ...
    func tripleTapDetectedInView(view _: UIView, touch _: UITouch) {}
}
